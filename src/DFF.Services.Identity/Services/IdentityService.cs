...
using DFF.Services.Identity.Messages.Events;
using DFF.Services.Identity.Repositories;
...
namespace DFF.Services.Identity.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task SignUpAsync(Guid id, string email, string password, string role = Role.User)
        {
            ...
            if (user != null)
            {
                throw new DFFException(Codes.EmailInUse,
        ...
        public async Task<JsonWebToken> SignInAsync(string email, string password)
        {
            ...
            {
                throw new DFFException(Codes.InvalidCredentials,
                    "Invalid credentials.");
            }
        ...
        public async Task ChangePasswordAsync(Guid userId, string currentPassword, string newPassword)
        {
            ...
            if (user == null)
            {
                throw new DFFException(Codes.UserNotFound,
                    $"User: '{userId}' was not found.");
            }
            if (!user.ValidatePassword(currentPassword, _passwordHasher))
            {
                throw new DFFException(Codes.InvalidCurrentPassword,
